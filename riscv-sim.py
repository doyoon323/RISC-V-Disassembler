...
def convert_bin(hcode):
    for i in range(32-len(hcode)):
        hcode = '0' + hcode
    return hcode
...
while True:
    ...
    bcode = [binary_data[i:i+4] for i in range(0, len(binary_data), 4)]
    
    bcode = bin(int(hcode,16))[2:].zfill(32)
    bcode = bcode.zfill(32)
    ...
    if disassembled_code == -1:
        ...
        print("inst",count,":",hcode,disassembled_code)
        count+=1
